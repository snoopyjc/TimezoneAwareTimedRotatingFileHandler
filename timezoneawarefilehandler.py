...
if __name__ == '__main__':
    ...
    new_handler = TimezoneAwareTimedRotatingFileHandler('test', when='midnight', utc=True, tzinfo=None)
    central_handler = TimezoneAwareTimedRotatingFileHandler('test', when='midnight', utc=True, tzinfo=central)
    central_4pm_handler = TimezoneAwareTimedRotatingFileHandler('test', when='midnight', utc=True, 
                                            tzinfo=central, atTime=dt_time(16, 0, 0))
    central_monday_handler = TimezoneAwareTimedRotatingFileHandler('test', when='W0', utc=True,
                                            tzinfo=central)
​
    _DAY = 24 * 60 * 60
    currentTime = int(time.time())
